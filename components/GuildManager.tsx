  ...
  return <div className="stack">
    ...
    <div className="grid-2">
      <div className="panel"><div className="panel-title"><span>⚔️ 최근 보스 기록</span><span className="muted">총 {data.records.length}건</span></div>
        {data.records.length ? <div className="table-wrap"><table className="dashboard-table"><thead><tr><th>날짜</th><th>보스</th><th>젠 시간</th><th>점수</th><th>참여</th></tr></thead><tbody>{data.records.slice(0, 7).map(r => <tr key={r.id}><td>{r.date}</td><td className="strong">{r.boss}</td><td>{r.spawn_time || "-"}</td><td>{formatNumber(r.score)}</td><td>{r.participants?.length || 0}명</td></tr>)}</tbody></table></div> : <Empty text="등록된 보스 기록이 없습니다." />}
      </div>
